...
class RVKFilteredTest extends PHPUnit_Framework_TestCase {	
	...
	public function setUp() {
		...
		$this->auto = new Autocompleter( new RVKFiltered( $snoopy, $this->filter ) );
		
		// Create autocompleter instance for 2 filters
		$this->auto2 = new Autocompleter( new RVKFiltered( $snoopy, $this->filter2 ) );
	}
	...
	public function testSubmitFiltered() {
		$resp = $this->auto->search( $this->queryFiltered );
		$this->assertEquals( $this->expectedFiltered, $resp );
	} 
	
	public function testSubmitFiltered2() {
		$resp = $this->auto2->search( $this->queryFiltered2 );
	...
	public function testMultiple() {
		$resp = $this->auto->search( 'test;' . $this->queryFiltered );
		$this->assertEquals( $this->expectedFiltered, $resp );
